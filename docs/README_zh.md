...
## 🚀 安装指南
> ⚠️ **兼容性声明**：Tab Organizer 仅支持 **Google Chrome** 浏览器。Firefox、Microsoft Edge、Safari 等其他浏览器暂不支持。
...
- **控制面板运行时 (`src/dashboard/`)**：
  - **编排器模式**：`App.tsx` 是全局状态的唯一入口。它负责执行原始浏览器标签与用户定义分组之间的“最终聚合”。
  - **模块懒加载**：像 `@dnd-kit` 这样的大型依赖只进入卡片视图的拖拽分块；表格视图不加载或渲染拖拽手柄。
