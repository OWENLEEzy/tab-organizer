...
import { parseSearchQuery } from '../newtab/hooks/useAppLogic';
...
describe('parseSearchQuery', () => {
  ...
  describe('Space Command parsing (/space:SpaceName, space:SpaceName)', () => {
    it('parses space filter with slash and argument', () => {
      expect(parseSearchQuery('/space:work')).toEqual({ type: 'space', arg: 'work', textQuery: '' });
      expect(parseSearchQuery('/space:social reddit')).toEqual({ type: 'space', arg: 'social', textQuery: 'reddit' });
    });

    it('parses slash-less space filter with argument', () => {
      expect(parseSearchQuery('space:personal')).toEqual({ type: 'space', arg: 'personal', textQuery: '' });
